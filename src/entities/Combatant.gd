...
func roll_attack_damage():
	if get_maximum_attack() == get_minimum_attack():
		return get_maximum_attack()
...
func inflict_damage(damage):
	...
	if player_id == Constants.PlayerId_Player:
		...
		if current_hp == 0:
			die()
	EventBus.emit_signal("create_damage_label", damage, get_target_position(), Color("B4202A"))

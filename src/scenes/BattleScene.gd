...
func start_battle(enemy_data):
	...
	for enemy in enemy_data:
		if i <= 3:
			enemy["position"] = get_node(str("EnemyCombatantPositions/Pos", i)).global_position
		else:
...
func prepare_battle(player_team, enemy_team):
	...
	var new_player_combatant = create_player_combatant()
	new_player_combatant.player_id = player_team.id
	player_combatants.append(new_player_combatant)
	# HealthBar
	var new_player_health_bar = health_bar_reference.instance()
	new_player_health_bar.init(new_player_combatant)
	new_player_health_bar.global_position = $PlayerCombatantPositions/Pos1.global_position
	new_player_combatant.connect("combatant_hp_changed", self, "combatant_hp_changed", [new_player_health_bar])
	new_player_combatant.connect("status_changed", self, "status_changed", [new_player_health_bar])
	new_player_combatant.connect("buffs_changed", self, "buffs_changed", [new_player_health_bar])
	$UI/HealthBars.add_child(new_player_health_bar)
	...
	for combatant in enemy_team.combatants:
		...
		if new_combatant != null:
			...
			new_health_bar.global_position = combatant.position
...
func create_player_combatant():
	var new_combatant = combatant_reference.instance()
	var combatant_position = $PlayerCombatantPositions/Pos1.global_position
	new_combatant.init(PlayerHandler.get_player_combatant(), combatant_position)
...
func start_turn():
	...
	turn_count += 1
	$UI/TurnCounter.text = str(turn_count)
	...
	# First add all player actions, then all enemies in order
	for combatant in $Combatants/Ysort.get_children():
		if combatant.player_id == player.id:
			if combatant.is_ko():
				continue
			for i in combatant.actions_per_turn:
...
func go_to_next_combatant():
	...
	if (initiative_order.size() > 0):
		...
		var current_combatant = initiative_order.pop_front()
		
		# show that combatant initiative hint
		current_combatant.bop()
...
func execute_action(acting_combatant, action, targets):
	...
	if action == null or targets.size() == 0:
		create_damage_label("Pass", acting_combatant.get_target_position(), Color("d3d3d3"))
		if !is_battle_over():
			go_to_next_combatant()
		return
...
func remove_combatant_from_queue(combatant):
	for initiative_object in initiative_order:
		if initiative_object == combatant:
...
func remove_combatant_ui(combatant):
	for health_bar in $UI/HealthBars.get_children():
...
func play_turn_count_tween():
	$UI/TurnCounter.show()
	$UI/TurnCounter/Anim.play("Bop")
